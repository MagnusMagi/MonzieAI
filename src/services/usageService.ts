...
class UsageService {
  ...
  private mapPlanToPackageKey(planIdentifier: string): string | null {
    ...
    if (lowerId.includes('yearly') || lowerId.includes('year') || lowerId.includes('annual'))
      return 'yearly';
  ...
  async canGenerateImage(userId: string): Promise<{ allowed: boolean; reason?: string }> {
    ...
    if (usage.dailyLimit > 0 && usage.dailyCount >= usage.dailyLimit) {
      return {
        allowed: false,
        reason: `You have reached your daily limit of ${usage.dailyLimit} photos. Please try again tomorrow.`,
      };
    }
    ...
    if (usage.count >= usage.limit) {
      return {
        allowed: false,
        reason: `You have reached your limit of ${usage.limit} photos for this period.`,
      };
    }
  ...
  async incrementUsage(userId: string): Promise<void> {
    try {
      ...
      if (currentUsage) {
        ...
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const lastResetDate = currentUsage.last_reset_date
          ? new Date(currentUsage.last_reset_date)
          : null;
        let newDailyCount = (currentUsage.daily_count || 0) + 1;
        const newLastResetDate = today.toISOString().split('T')[0];
